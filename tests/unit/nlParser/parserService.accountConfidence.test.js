...
const AI_SYNTHETIC_ACCOUNT_MAPPING = 0.5; // the AI's own uncalibrated self-report
...
function setupMocks({ journalEntries }) {
  normalizeExtraction.mockReturnValue({
    ...
    confidence: { intent: 0.9, amount: 0.9, date: 0.9, accountMapping: AI_SYNTHETIC_ACCOUNT_MAPPING },
  });
...
describe('parserService — real account-resolution confidence (Phase 2)', () => {
  ...
  test('overrides the synthetic accountMapping score with real resolution confidence when both legs match exactly', async () => {
    ...
    expect(result.confidence.accountMapping).not.toBe(AI_SYNTHETIC_ACCOUNT_MAPPING);
  });
  ...
  test('falls back to the synthetic AI confidence when no live accounts are available (no regression)', async () => {
    setupMocks({
      journalEntries: [
        { account: 'Cash at Bank', entryType: 'credit', amount: 100 },
        { account: 'Rent', entryType: 'debit', amount: 100 },
    ...
    expect(result.confidence.accountMapping).toBe(AI_SYNTHETIC_ACCOUNT_MAPPING);
  });
